...
export class HomePageService {
  ...
  getProducts():Observable<IProduct[]>{
    let tempVar = this.http.get<IProduct[]>('https://quickkart-webservice20.azurewebsites.net/api/home/GetProducts')
    console.log(tempVar)
    return tempVar
  }
  ...
  MakePayment(CardNumber1:string,cvv1:string,ex:string,pid:number,cost:number):Observable<boolean>{
    ...
    let tempVar = this.http.post<boolean>('http://localhost:44340/api/PaymentFunction',pay)
    return tempVar
  }
  ...
  ValidateUser(userEmailID:string, userPassword:string, type:string):Observable<number>
  {
    ...
    let result=this.http.post<number>('https://azurefunctionloginservice.azurewebsites.net/api/LoginFunction?code=cWDXGwytS4CxlLtWH8OYYnzXHEsnBD-GEMIXlxyIit-TAzFuYuj9DQ==',user)
    return result
